...
def get_a_and_c_genes(todo, db):
    ...
    for sample in todo.keys():
        ...
        sql = """SELECT genen FROM panels_cgenen
        WHERE (panel="{}" AND versie={})
        """.format(panel, versie)
        c.execute(sql)
        ...
        todo[sample]['agenen'] = a_genes
        todo[sample]['cgenen'] = c_genes
    conn.close()
    return todo


def get_other_agenes(todo, db):
    conn = sqlite3.connect(db)
    c = conn.cursor()
    for sample in todo.keys():
